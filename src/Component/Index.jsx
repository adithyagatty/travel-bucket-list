...
export default function Index() {
    ...
    useEffect(() => {
        ...
        let bin = localStorage.getItem('BinDestiny') ? JSON.parse(localStorage.getItem('BinDestiny')) : []
        setBinValue(bin)
    ...
    const Deletedestiny = async (item) => {
        if (item.pinned) {
            ...
            let value = (binValue.filter((e) => {
                return e !== item
            }))
            setBinValue(value)
            localStorage.setItem('BinDestiny', JSON.stringify(value))
            await setOpen(false)
            await setOpenDelete(false)
        }
        else {
    ...
    const RemoveBin = (item) => {
        console.log('HelloPin', item)
        let makepinfalse = item.bin = false
        console.log('makepinfalse', makepinfalse)
        let value = (binValue.filter((e) => {
            return e !== item
        }))
        setBinValue(value.length > 0 ? value.reverse() : [])
        localStorage.setItem('BinDestiny', JSON.stringify(value.length > 0 ? value.reverse() : []))
    ...
    const tempDelete = (item) => {
        console.log('delete', item)
        let makebined = item.bin = true
        console.log('MAkeTRue', makebined)
        setBinValue([...binValue, item])
        let bin = [...binValue, item];
        let value = (destinyList.filter((e) => {
            return e !== item
        }))
        ...
        localStorage.setItem('BinDestiny', JSON.stringify(bin))
        ...
        <>
            <div className="main-container">
                <div className="navbar">
                    ...
                    <h1 style={{ fontWeight: 'normal', position: 'relative' }}>Travel Bucket List</h1>
                ...
                <div className="GRID-div">
                    ...
                    {sidebar && (
                        <div className='sidebar'>
                            <div >
                                <p style={{ cursor: "pointer", ...mystyle }} onClick={() => { return (setInsertTrue(false), setShowBin(false)) }}> <HomeIcon />  <span>Home</span></p>
                                {/* <p><PhoneIcon />  <span>Phone</span></p> */}
                                <p style={{ cursor: "pointer", marginTop: "-20px", ...mystyle }} onClick={mainTag}><AddLocationIcon />  <span>Add Destination</span></p>
                                <p style={{ cursor: "pointer", marginTop: "-20px", ...mystyle }} onClick={() => { return setShowBin(true), setInsertTrue(false) }}><DeleteIcon />  <span>Bin</span></p>
                                {/* <p><FavoriteIcon />  <span>Famous Place</span></p> */}
                                <p onClick={Logout} style={{ cursor: "pointer", position: 'absolute', bottom: '10px' }}>  <LogoutIcon /><span>Logout</span></p>
                        ...
                        <div className="All-map">
                            {!insertTrue && showBin == true &&
                                <div className="PinnedDestiny">
                                    ...
                                    {binValue.length == 0 ? "NO Data" :
                                        binValue?.map((item, index) => {
                                            return (
                                                <>
                                                    <div key={index}>
                                                        <div ref={card} className="card" style={{backgroundImage:`url(${item?.image})`}} >



                                                            <span style={{ position: 'relative', float: 'right', left: '-12px' }}>
                                                                <Tooltip title='Remove from Bin'>
                                                                    <UndoIcon onClick={() => RemoveBin(item)} className='Pin' style={{ color: 'black' }} />
                                                                </Tooltip>

                                                                <Tooltip title='Remove from Bin'>
                                                                    <DeleteIcon onClick={() => handleDeleteIcon(item)} className='Pin' style={{ color: 'red' }} />
                                                                </Tooltip>
                                                            </span>
                                                            <div onClick={() => handleCard(item)} className='card-item'>
                            ...
                            {!insertTrue && showBin == false &&
                                <div className="card-container">
                                    ...
                                    <h4 style={{ marginTop: "0px", display: 'block', width: '100%', marginLeft: '20px', color: '#7a7272', fontSize: '13px' }}>
                                        OTHER
                                            ...
                                            <>
                                                <div key={index}>
                                                    <div className="card" style={{backgroundImage:`url(${item?.image})`}}>
                                                        ...
                                                        <span style={{ position: 'relative', float: 'right', left: '-12px' }}>
                                                            <Tooltip title='Delete destiny'>
                                                                <DeleteIcon onClick={() => tempDelete(item)} className='Pin' style={{ color: 'red' }} />
                                                            </Tooltip>
                ...
                <Dialog
                    fullWidth
                    style={{ height: '600px', overflowWrap: 'anywhere', }}
                    open={open}
                    onClose={handleClose}
                ...
                >
                    ...
                    <DialogContent>
                        ...
                        <DialogContentText id="alert-dialog-description">
                            <span style={{ fontSize: '20px' }}>Famous Place:</span>
                            <br />
